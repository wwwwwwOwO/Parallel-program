...
int main(void) {
   ...
   bin_counts = malloc(bin_count*sizeof(int));     // 全局
   loc_bin_cts = malloc(bin_count*sizeof(int));    // 局部
   data = malloc(data_count*sizeof(float));        // 全局
   local_data = malloc(local_data_count*sizeof(float));     // 局部
...
int Which_bin(float data, float bin_maxes[], int bin_count,
   ...
   /*******************************************************************/
      ...
      //The first bin
      if (data >= min_meas && data < bin_maxes[0])
         return i;

      //Get the right bin
      while (i < bin_count && data >= bin_maxes[i] && data >= min_meas)
      {
         ++i;
      }
...
void Set_bins(
   ...
   if (my_rank == 0) {
      ...
      // Initialize loc_bin_cts and bin_maxes
      for (i = 0; i < bin_count; i++) {
         loc_bin_cts[i] = 0;
